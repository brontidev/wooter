...
const domainRouter = wooter.router(c.chemin(c.pString("domain")))

domainRouter.route(c.chemin(c.pMultiple(c.pString("uid"), false)), {
	"GET": async ({ resp, params }) => {
	...
	"POST": async ({ request, resp, params, url }) => {
		const domain = encodeURI(params.get("domain"))
		const uid = params.get("uid")
