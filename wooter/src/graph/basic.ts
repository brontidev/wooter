import { type IChemin, type ICheminMatch, match as matchChemin, matchExact as matchCheminExact, splitPathname } from "@/export/chemin.ts"

export type Matcher<Node, FindData> = (match: ICheminMatch<unknown>, data: FindData) => Node | undefined

export class CheminGraph<Node, FindData> {
    private nodes = new Set<{ path: IChemin, node: Node }>()

    private subgraphs = new Set<{ path: IChemin, match: Matcher<Node, FindData> }>()
    ...
    protected getNode(pathname: string | string[], data: FindData): { params: unknown, node: Node } | undefined {
