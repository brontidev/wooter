...
export type Matcher<T, Options = undefined> = (path: string[], current_index: number, options: Options) => ({ ok: false } | { ok: true, value: T, consume: number })
...
export type ParamsOfPath<P extends readonly unknown[]> =
    P extends readonly [infer Head, ...infer Tail]
    ? [Head] extends [Param<infer Name, any>]
    ? Merge<{ [K in Name]: ParamValue<Head> }, ParamsOfPath<Tail>>
    : ParamsOfPath<Tail>
    : {};