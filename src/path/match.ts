...
export function match<const TPath extends Path>(
	...
	pattern: TPath,
	exact: true,
): { match: false } | { match: true; params: ParamsOfPath<TPath> }
export function match<const TPath extends Path>(
	segments: string[],
	i: number,
	pattern: TPath,
	exact: false,
): { match: false } | { match: true; params: ParamsOfPath<TPath>; stopped_at: number }
export function match(
	segments: string[],
	i: number,
	pattern: Path,
	exact: boolean,
): { match: false } | { match: true; params: Record<string, unknown>; stopped_at?: number } {
