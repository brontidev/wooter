...
const wooter = new Wooter()
    .use(async ({ request, resp, up }) => {
        try {
            ...
            if (e instanceof Redirect) {
                return resp(redirectResponse(e.location, {
                    status: e.status
    ...
    .use<{ cookies: Cookies }>(async ({ request, resp, up }) => {
        ...
        const cookieMap: Record<string, { value: string, opts?: Partial<SerializeOptions> }> = {}

        const cookies: Cookies = {
            get: (name: string) => cookieMap[name].value ?? parsedCookies[name],
            getAll: () => Object.fromEntries(Object.entries(parsedCookies).concat(Object.entries(cookieMap).map(([name, { value }]) => {
                return [name, value]
            }))),
            delete: (name: string) => { cookieMap[name] = { value: '', opts: { maxAge: 0 } } },
            set: (name: string, value: string, options?: Partial<SerializeOptions>) => { cookieMap[name] = { value, opts: options } }
        };
        ...
        const newCookies = Object.entries(cookieMap)
            ...
            .filter(([_, cookie]) => cookie && typeof cookie === 'object' && 'name' in cookie && 'value' in cookie)
...
wooter
    ...
    .namespace(chemin("auth"), wooter => {
        wooter.GET(chemin("login"), async ({ request, resp, err, data: { cookies } }) => {
            let json = await request.json()
            if (!json.username || !json.password) return resp(errorResponse(400, "Missing username or password"))
            if (json.username !== "admin" || json.password !== "admin") return resp(errorResponse(401, "Invalid username or password"))
            cookies.set("session", "example", {
                httpOnly: true,
                sameSite: "lax",
                maxAge: 86400 * 7,
                path: "/"
    ...
    .namespace(chemin("api"), wooter => {
        wooter.GET('/gateway', async ({ request, resp, err, data: { username } }) => {
