...
const basicAuth = middleware<{ username: string }>(async ({ request, resp, expectAndRespond }) => {
...
const json = middleware<{ json: () => Promise<any> }>(async ({ request, resp, expectAndRespond }) => {
    let _json: any
    let _jsonFetched = false
    
    await expectAndRespond({
        json: async () => {
            if (_jsonFetched) return _json
...
```ts
const requestId = middleware<{ requestId: string }>(async ({ expectResponse, resp }) => {
    const id = crypto.randomUUID()
    
    const response = await expectResponse({ requestId: id })
