...
const wooter = new Wooter()
	.use<{ session?: { id: string; createdAt: Date } }>(
		async ({ request, up, resp }: { request: Request }) => {
...
wooter.route(chemin("book", pNumber("id")))
	...
	.PUT(async ({ param: { id }, request, resp }) => {
		const body = await request.json()
	...
	.PATCH(async ({ param: { id }, request, resp }) => {
		const body = await request.json()
		const bookEntry = await db.get<Book>(["books", id])
		if (!bookEntry.value) {
			return resp(errorResponse(404, "Book not found"))
